export default function HeroImage(props: { title: string; imageUrl: string }) {
  const imageAvifUrl =
    props.imageUrl.replace("/", "/avif/").split(".")[0] + ".avif";
  return (
    <div
      className="flex items-center justify-center relative bg-cover bg-center mb-4 md:mb-12 lg:mb-24 h-[20vh] lg:h-[42vh]"
      style={{
        backgroundImage: `image-set(url('${imageAvifUrl}') type('image/avif'), url('${props.imageUrl}') type('image/jpeg'))`,
    ...
    >
      ...
      <div className="relative z-10 text-center text-white">
        <h1 className="text-3xl lg:text-6xl font-bold mb-4">{props.title}</h1>
      </div>
